          ...
          <div className="space-y-6">
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-3xl p-6">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div><Label>Email</Label><Input className="mt-1 bg-card" type="email" placeholder="you@email.com" /></div>
                <div><Label>Phone</Label><Input className="mt-1 bg-card" placeholder="+1 415 555 0140" /></div>
                <div><Label>Emergency contact</Label><Input className="mt-1 bg-card" placeholder="Contact + phone" /></div>
                <div className="sm:col-span-2"><Label>Address</Label><Input className="mt-1 bg-card" placeholder="Street, city, country" /></div>
                <div>
                  <Label>Nationality</Label>
                  <Select><SelectTrigger className="mt-1 bg-card"><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>{["USA","UK","Canada","Italy","Japan","Australia","Other"].map(c=><SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
            ...
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass rounded-3xl p-6">
              ...
              <Tabs defaultValue="card" className="mt-4">
                ...
                <TabsContent value="card" className="mt-5 grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2"><Label>Cardholder name</Label><Input className="mt-1 bg-card" placeholder="Name on card" /></div>
                  <div className="sm:col-span-2"><Label>Card number</Label><Input className="mt-1 bg-card" placeholder="1234 5678 9012 3456" /></div>
                  <div><Label>Expiry</Label><Input className="mt-1 bg-card" placeholder="MM / YY" /></div>
                  <div><Label>CVC</Label><Input className="mt-1 bg-card" placeholder="123" /></div>
                  <div className="sm:col-span-2 flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1"><Lock className="h-3.5 w-3.5" />256-bit encryption</span>
              ...
              <Button size="lg" className="mt-6 w-full gap-2 bg-forest text-forest-foreground shadow-glow hover:bg-forest/90">
                <ShieldCheck className="h-4 w-4" /> Pay ${total.toFixed(2)} securely
              </Button>
            </motion.section>
          </div>
